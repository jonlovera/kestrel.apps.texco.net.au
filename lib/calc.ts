/**
 * Bonus pool calculation engine, ported 1:1 from the prototype
...
export function computeScalesAndBonuses(
  ...
  emps.forEach((e) => {
    if (e.sm) {
      ...
      empLockedVp += e.finalBonus * e.vp;
      empLockedNp += e.finalBonus * e.np;
    } else {
  ...
  const vicBaseScale =
    vicBaseDenom !== 0
      ? Math.max(0, (caps.vCap - empLockedVp - shared.sharedDaVp) / vicBaseDenom)
      : 1;
  const nswBaseScale =
    nswBaseDenom !== 0
      ? Math.max(0, (caps.nCap - empLockedNp - shared.sharedDaNp) / nswBaseDenom)
      : 1;
  ...
  // Step 4: state scale — DA only redistributes among state employees
  const vicScale =
    empBipmVpUnlocked !== 0
      ? Math.max(0, (stateVicAvail - empLockedVp - empDaVpUnlocked) / empBipmVpUnlocked)
      : 1;
  const nswScale =
    empBipmNpUnlocked !== 0
      ? Math.max(0, (stateNswAvail - empLockedNp - empDaNpUnlocked) / empBipmNpUnlocked)
      : 1;
