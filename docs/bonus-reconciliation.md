...
1. **The four part-split staff — RESOLVED IN CODE (25 Aug 2026, later the same day).** The sheet's state pools are defined *net* of these four (their locked amounts are the "split state" carve), so a pool-bound VIC must not also carry their payouts in its home total. Rather than move them back to `SHARED`, the app now treats any row whose cost splits across both pools as **carve-funded** (`lib/calc.ts` `isCarveFunded` / `inStateHomeTotal`): excluded from every home-state total a state pool is measured against — the cards (`poolCardTotals.vicHome`), `capRoom`/gate 4, a whole-state lead's Allocated (`lib/manager-pool.ts` `countsAgainstPool`, sent to the browser as `ScopedRow.inHomeTotal`), the redistribution budget and the impact dialog — and bounded like a Shared Services row (group cap only). They stay `st = "VIC"` for display. Result at this data: **VIC Remaining +9,736, NSW Remaining +5,084**, i.e. the sheet's ledgers; no payout moved. Open for Dee only in the sense of confirming the treatment.
...
7. **Part-split split method** for the Shared Services card lines — **display resolved 25 Aug 2026:** the card now shows the typed carve constants **87,637 / 25,239** (`FY26_CARVE_OUTS`, the same figures the state cards print as "Less split state"). The *computed* attribution (`poolCardTotals.vicPartSplit/nswPartSplit`, 90,050 / 23,959) is kept for reconciliation only. Still open: whether the computed figure should use the sheet's no-locks scale (`I13`) — a small engine change — or the sheet should restate F9/F10 on the with-locks method.
